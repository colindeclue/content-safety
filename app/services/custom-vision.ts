...
export const uploadImagesOnce = async () => {
    ...
    const trainingCredentials = new msRest.ApiKeyCredentials({
    ...
    if (!projectExists) {
        ...
        project = projectExists
            ? existingProjects.find((project) => project.name === projectName)!
            : await trainingClient.createProject(projectName);
        ...
        const tags = await Promise.all(
            tagNames.map((name) => trainingClient.createTag(project!.id!, name))
        );
        ...
        for (const tag of tags) {
            ...
            if (!fs.existsSync(tagDir)) {
                console.warn(
                    `Directory for tag ${tag.name} does not exist: ${tagDir}`
                );
                continue;
            }
            ...
            for (const file of files) {
                ...
                await retry(10, 5000, () =>
                    trainingClient.createImagesFromData(
                        project!.id!,
                        imageData,
                        {
                            tagIds: [tag.id!],
                ...
                if (count === 2) {
                    count = 0;
                    console.log(
                        "Waiting for 1 second to avoid rate limiting..."
                    );
        ...
        let trainingIteration = await retry(3, 10000, () =>
            trainingClient.trainProject(project!.id!)
        );
        while (trainingIteration.status === "Training") {
            console.log("Training in progress...");
            await setTimeoutPromise(10000, null); // Wait for 5 seconds
            trainingIteration = await trainingClient.getIteration(
                project!.id!,
